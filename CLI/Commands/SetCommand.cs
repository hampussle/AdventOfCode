...
internal class SetCommand : Command<SetCommand.Settings>
{
    public sealed class Settings : CommandSettings
    {
        ...
        [Description("Set the test input to use when running command <run -t>")]
        [CommandArgument(2, "<TESTINPUT>")]
    ...
    public override int Execute(CommandContext context, Settings settings, CancellationToken cancellationToken)
    {
        ...
        if (settings.TestInput is string testInput && settings.Year is int year && settings.Day is int day)
        {
